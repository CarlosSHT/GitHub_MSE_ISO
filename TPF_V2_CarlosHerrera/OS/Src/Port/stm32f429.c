...
* Private defines
*******************************************************************************/
#define IS_inInterrupt()	(__get_IPSR() != 0U)	/// funcion para reconocer
 	 	 	 	 	 	 	 	 	 	 	 	 	/// si esta dentro de una
 	 	 	 	 	 	 	 	 	 	 	 	 	/// interrupcion
...
* Public Variables
*******************************************************************************/
osIRQVector irqVector[IRQ_NUMBER] = { 0 };		///	Vector de interrupcion del OS
...
{
	IRQHandler cbFunc = NULL;

	bool_t irq_context =false;

	osKernel_SaveState();

	osSetIRQState();

	cbFunc = irqVector[irqType].handler;	/// Asigna la función callback correspondiente
	if (cbFunc!=NULL) {						/// Si existe una funcion asignada entonces ejecuta
		cbFunc(irqVector[irqType].data);	/// Ejecuta la funcion callback con el argumento "data"
	}

	osKernel_RecoverState();

	NVIC_ClearPendingIRQ(irqType);			/// Limpia las interrupciones pendientes

	if (osKernel_GETschdlrINirq()) {
		osKernel_UNSETschdlrINirq();
		osKernel_YieldCPU();
	}
}
