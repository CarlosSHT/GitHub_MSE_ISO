...
typedef struct
{
	uint8_t		data_bytes[MAX_SIZE_QUEUE];	/// Lista de punteros de datos en la cola
	uint8_t		count_space;						/// Puntero inicial de la cola
	uint8_t		max_count;					/// Puntero inicial de la cola
	uint8_t		index_fill;					/// Indice de dato a insertar
	uint8_t		index_get;					/// Indice de dato a sacar
	uint32_t	data_size;					/// tamaño en bytes del dato a encolar
	osTaskObject	*task;					/// Tarea en ejecución
	 osTaskObject *blocked_list;
}osQueueObject;
