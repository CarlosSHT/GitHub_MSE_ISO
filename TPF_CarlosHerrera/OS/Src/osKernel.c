...
typedef enum
{
	OS_FROM_RESET = 0,				//inicio luego de un reset
...
//    osTaskObject*   listTask[MAX_NUMBER_TASK];  // Task list.
	...
	int8_t			nest_critcntr;			// contador para anidacion de secciones criticas
	bool_t			request_scheduling;
} osKernelObject;
...
static void scheduler(void)
{
    ...
    if (osKernel.current_osState == OS_FROM_RESET) {

    	osKernel.current_osState=OS_NORMAL_RUN;
    ...
    for (n_priority = FIRST_INDEX_TASK_PRIORITY; n_priority < MAX_NUMBER_PRIORITY; ++n_priority) {
...
void setKernelOSstate(os_State state)
{
	osKernel.current_osState=state;
}

os_State getKernelOSstate(void)
{
...
void osStart(void) {
	...
	osKernel.request_scheduling=false;
	osKernel.current_osState=OS_FROM_RESET;
	osKernel.running = false;
	osKernel.currentTask = NULL;
	osKernel.nextTask = NULL;
	osKernel.nest_critcntr = 0;
	CreateIdleTask();
	/*
...
void osEnterCriticalSection(void)
 {
	...
	osKernel.nest_critcntr++;	/// Incrementa el contador de anidacion critica
}
...
void osExitCriticalSection(void)
 {
	osKernel.nest_critcntr--;			///	El contador de anidaciones disminuye
	if (osKernel.nest_critcntr < 1) {	///	y verifica si ya no hay mas anidacion
		osKernel.nest_critcntr = 0;		///	para vovler a activar las interrupciones
		__enable_irq();
	}
}

bool_t osNeedIRQScheduling(void)
{
	return osKernel.request_scheduling;
}

void osSetIRQScheduling(void)
{
	osKernel.request_scheduling=true;
}

void osUnsetIRQScheduling(void)
{
	osKernel.request_scheduling=false;
}

void osSetIRQState(void)
{
	osKernel.current_osState=OS_IRQ_RUN;
}

void osSetNormalState(void)
{
	osKernel.current_osState=OS_NORMAL_RUN;
}

void osSaveCurrentState(void)
{
	osKernel.aux_osState=osKernel.current_osState;
}

void osRecoverState(void)
{
	osKernel.current_osState=osKernel.aux_osState;
}

bool_t osIsIRQState(void)
{
	bool_t ret=false;
	if (osKernel.current_osState==OS_IRQ_RUN) {
		ret = true;
	}
	return ret;
}
