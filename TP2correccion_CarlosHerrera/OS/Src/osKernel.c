...
//    osTaskObject*   listTask[MAX_NUMBER_TASK];                              // Task list.
    ...
    osTaskObject*   priority_matrix[MAX_NUMBER_PRIORITY + 1][MAX_NUMBER_TASK];	// Matriz de prioridad con los punteros de las tareas
...
//    }
    ...
    osKernel.priority_matrix[priority][osKernel.ntask_priority[priority]]=handler;
...
void osDelay(const uint32_t tick)
{
	...
	running_task=getRunningTask();	// Utilizo busqueda de tarea Running y no a traves del currentTask debido
									// a que hay un punto borde donde la tarea actual puede perder su condición
									// de estado Running
...
static void countTaskDelay(void)
{
	...
	for (int var = 0; var < MAX_NUMBER_PRIORITY; ++var) {
		for (int var2a = 0; var2a < MAX_NUMBER_TASK; ++var2a) {
			...
			if (osKernel.priority_matrix[var][var2a]->delay_time == 0 && osKernel.priority_matrix[var][var2a]->state==OS_TASK_BLOCK) {
